/*
    I2S Bluetooth Transmitter (based on https://github.com/schreibfaul1/ESP32-audioI2S/tree/master/examples/I2S%20Bluetooth%20Transmitter)
...
AudioFiFoBuffer fifoBuffer;

char BT_SINK_NAME[] = "JBL Charge 3";  // set your sink devicename here
...
void i2s_install() {
    ...
    i2s_std_cfg.clk_cfg.sample_rate_hz = 44100;                 // 44800;
...
int32_t get_data(uint8_t *data, int32_t bytes) {
    return fifoBuffer.getData(data, bytes);  // Retrieves the exact data required
}
...
void setup() {
    ...
    fifoBuffer.setChannelHandle(i2s_rx_handle);
}

//----------------------------------------------LOOP--------------------------------------------------------------------
void loop() {
    fifoBuffer.loopWriteBuf();
    vTaskDelay(1);
}
